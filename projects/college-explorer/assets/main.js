...
const parseDatum = (d) => {
  return (
    toLocaleType(d["Locale"]),
    toLocaleSize(d["Locale"]),
    [
      d["Name"],
      d["Control"],
      d["Region"],
      ...
      +d["% Part-time Undergrads"],
      +d["Average Cost"],
      +d["Expenditure Per Student"],
      +d["Average Faculty Salary"],
      +d["% Full-time Faculty"],
      +d["% Undergrads with Pell Grant"],
      +d["Completion Rate 150% time"],
      +d["Retention Rate (First Time Students)"],
      +d["% Undergrads 25+ y.o."],
      +d["3 Year Default Rate"],
      +d["Median Debt"],
      +d["Median Debt on Graduation"],
      +d["Median Debt on Withdrawal"],
      +d["% Federal Loans"],
      +d["% Pell Grant Recipients"],
      +d["Average Age of Entry"],
      +d["Average Family Income"],
      +d["Median Family Income"],
      +d["Poverty Rate"],
      +d["Number of Unemployed 8 years after entry"],
      +d["Number of Employed 8 years after entry"],
      +d["Mean Earnings 8 years After Entry"],
      +d["Median Earnings 8 years After Entry"],
    ]
  );
};
...
const localeDistance = (d1, d2) => {
  return (
    LOCALE_SIZE_DISTANCE(d1[LOCALE_SIZE_INDEX], d2[LOCALE_SIZE_INDEX]) +
...
function setTextBoxes(d) {
  ...
  document.getElementById("acceptance_rate").innerHTML = (d[4] * 100).toFixed(
    2
  );
  ...
  if (!visible.includes(key))
    document.getElementById("active-key").innerHTML =
      d[indexOfKey(key)].toFixed(4);
}
...
function startGraph() {
  ...
  if (!visible.includes(key))
    document.getElementById("active-key-label").innerHTML = key + ":";
  if (visible.includes(key))
    document.getElementById("active-key-label").innerHTML = "";
  ...
  const cost = svg.append("path").attr("fill", "#aaa");
  ...
  dists = data.map((d1) =>
    data.map((d2) => generalDistanceFunction(d1, d2, index))
  );
  ...
  simulation = d3
    ...
    .force(
      "collide",
      d3.forceCollide().radius((d) => 1.5 + scaleSize(d[SIZE_INDEX]))
    )
    .on("tick", function () {
  ...
  var brushContainer = svg.append("g").attr("id", "brush-container");
  ...
  function handleBrushMove() {
    ...
    var total_sat = 0;
    var total_key = 0;
    d3.selectAll(".dot").classed("dot-not-dragged", function (d) {
      var cx = d.x;
      var cy = d.y;
    ...
    document.getElementById("average_cost").innerHTML = numberWithCommas(
      avg_cost.toFixed(0)
    );
    document.getElementById("act").innerHTML = avg_act.toFixed(0);
    document.getElementById("sat").innerHTML = avg_sat.toFixed(0);
    if (!visible.includes(key))
      document.getElementById("active-key").innerHTML = avg_key.toFixed(4);
  }
